...
BEGIN
  ...
  for v_cur in (select * from zsi_bpartner) 
  LOOP
    ...
    LOOP
        ...
        insert into c_bpartner_location(c_bpartner_location_id,c_location_id,c_bpartner_id,c_tax_id,c_salesregion_id,
                                        AD_CLIENT_ID, AD_ORG_ID, CREATED, CREATEDBY, UPDATED, UPDATEDBY,
                                        phone,phone2,fax,isshipto,isbillto,isremitto,istaxlocation,isheadquarter,uidnumber,name)
               values(get_uuid(),v_locid,v_bpid,v_tax,v_salesregion,ad_client,v_org,now(),creator,now(),creator,
    ...
    select c_paymentterm_id into v_paymentterm from  c_paymentterm where  name=(select paymentterm_key from zsi_bp_customer where bp_value_key=v_cur.value);
    ...
    update c_bpartner set iscustomer=cc.iscustomer,invoicerule=cc.invoicerule,c_invoiceschedule_id=v_invoiceschedule,invoicegrouping=cc.invoicegrouping,deliveryrule=cc.deliveryrule,deliveryviarule=cc.deliveryviarule,m_pricelist_id=v_pricelist,
                      ...
                      from (select iscustomer,invoicerule,invoicegrouping,deliveryrule,deliveryviarule,paymentrule,so_creditlimit,fixmonthday,fixmonthday2,fixmonthday3 
    ...
    select c_paymentterm_id into v_paymentterm from c_paymentterm where  name=(select po_paymentterm_key from zsi_bp_vendor where bp_value_key=v_cur.value);
    --Update Vendor
    ...
    LOOP
        select c_country_id into v_country from c_country where countrycode=v_cur2.country_key;
        insert into c_bp_bankaccount (c_bp_bankaccount_id,c_bpartner_id,c_country_id,AD_CLIENT_ID, AD_ORG_ID, CREATED, CREATEDBY, UPDATED, UPDATEDBY,
                                     isactive,accountno,showaccountno,routingno,iban,showiban,swiftcode,bank_name,a_name)
