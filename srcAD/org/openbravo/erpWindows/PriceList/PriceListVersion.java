...
/*
  String deleteRelation(VariablesSecureApp vars, String strM_PriceList_Version_ID, String strPM_PriceList_ID) throws IOException, ServletException {
    log4j.debug("Deleting records");
    Connection conn = this.getTransactionConnection();
    try {
      if (strM_PriceList_Version_ID.startsWith("(")) strM_PriceList_Version_ID = strM_PriceList_Version_ID.substring(1, strM_PriceList_Version_ID.length()-1);
      if (!strM_PriceList_Version_ID.equals("")) {
        strM_PriceList_Version_ID = Replace.replace(strM_PriceList_Version_ID, "'", "");
        StringTokenizer st = new StringTokenizer(strM_PriceList_Version_ID, ",", false);
        while (st.hasMoreTokens()) {
          String strKey = st.nextToken();
          if (PriceListVersionData.deleteTransactional(conn, this, strKey, strPM_PriceList_ID)==0) {
...
*/
  private PriceListVersionData getEditVariables(Connection con, VariablesSecureApp vars, String strPM_PriceList_ID) throws IOException,ServletException {
    ...
    try {
    data.mDiscountschemaId = vars.getStringParameter("inpmDiscountschemaId");     data.adClientId = vars.getRequestGlobalVariable("inpadClientId", windowId + "|AD_Client_ID");     data.adClientIdr = vars.getStringParameter("inpadClientId_R");     data.mPricelistVersionGenerate = vars.getStringParameter("inpmPricelistVersionGenerate");     data.adOrgId = vars.getRequestGlobalVariable("inpadOrgId", windowId + "|AD_Org_ID");     data.adOrgIdr = vars.getStringParameter("inpadOrgId_R");     data.mPricelistId = vars.getStringParameter("inpmPricelistId");     data.mPricelistIdr = vars.getStringParameter("inpmPricelistId_R");     data.name = vars.getStringParameter("inpname");     data.description = vars.getStringParameter("inpdescription");     data.validfrom = vars.getStringParameter("inpvalidfrom");     data.mPricelistVersionId = vars.getRequestGlobalVariable("inpmPricelistVersionId", windowId + "|M_PriceList_Version_ID");     data.proccreate = vars.getStringParameter("inpproccreate");     data.mPricelistVersionBaseId = vars.getStringParameter("inpmPricelistVersionBaseId");     data.isactive = vars.getStringParameter("inpisactive", "N"); 
   ...
   private PriceListVersionData[] getRelationData(PriceListVersionData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mDiscountschemaId = FormatUtilities.truncate(data[i].mDiscountschemaId, 44);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 44);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 44);        data[i].mPricelistId = FormatUtilities.truncate(data[i].mPricelistId, 44);        data[i].name = FormatUtilities.truncate(data[i].name, 50);        data[i].description = FormatUtilities.truncate(data[i].description, 50);        data[i].mPricelistVersionId = FormatUtilities.truncate(data[i].mPricelistVersionId, 10);        data[i].mPricelistVersionBaseId = FormatUtilities.truncate(data[i].mPricelistVersionBaseId, 44);}
    }
    return data;
  }
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPM_PriceList_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|M_PriceList_ID", data[0].mPricelistId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);
  ...
  private void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strPM_PriceList_ID, String strM_PriceList_Version_ID, TableSQLData tableSQL)
    ...
    ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "PriceListVersion", false, "document.frmMain.inpmPricelistVersionId", "grid", "..", "".equals("Y"), "PriceList", strReplaceWith, false);
...
//    }
    ...
    try {
      ...
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "PriceListVersion_Relation.html", "PriceList", "W", strReplaceWith, tabs.breadcrumb());
  ...
  private void printPageEdit(HttpServletResponse response, HttpServletRequest request, VariablesSecureApp vars,boolean boolNew, String strM_PriceList_Version_ID, String strPM_PriceList_ID, TableSQLData tableSQL)
    ...
    if (dataField==null) {
      if (boolNew || data==null || data.length==0) {
        ...
        data = PriceListVersionData.set(strPM_PriceList_ID, Utility.getDefault(this, vars, "Description", "", "146", "238", "", dataField), Utility.getDefault(this, vars, "AD_Org_ID", "@AD_Org_ID@", "146", "238", "", dataField), Utility.getDefault(this, vars, "M_Pricelist_Version_Base_ID", "", "146", "238", "", dataField), "Y", Utility.getDefault(this, vars, "ProcCreate", "", "146", "238", "N", dataField), Utility.getDefault(this, vars, "Name", "@#Date@", "146", "238", "", dataField), Utility.getDefault(this, vars, "M_Pricelist_Version_Generate", "N", "146", "238", "N", dataField), Utility.getDefault(this, vars, "CreatedBy", "", "146", "238", "", dataField), PriceListVersionData.selectDef2992_0(this, Utility.getDefault(this, vars, "CreatedBy", "", "146", "238", "", dataField)), "", Utility.getDefault(this, vars, "ValidFrom", "@#Date@", "146", "238", "", dataField), Utility.getDefault(this, vars, "UpdatedBy", "", "146", "238", "", dataField), PriceListVersionData.selectDef2994_1(this, Utility.getDefault(this, vars, "UpdatedBy", "", "146", "238", "", dataField)), Utility.getDefault(this, vars, "M_DiscountSchema_ID", "", "146", "238", "", dataField), Utility.getDefault(this, vars, "AD_Client_ID", "@AD_Client_ID@", "146", "238", "", dataField));
        
      }
    } else {
      data = new PriceListVersionData[1];
      java.lang.Object  ref1= dataField;
      data[0]=(PriceListVersionData) ref1;
      data[0].created="";
      data[0].updated="";
    }
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPM_PriceList_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = SequenceIdData.getUUID();
          ...
          CrudOperations.UpdateCustomFields(tabId, data.mPricelistVersionId, vars, this);
        } catch(Exception ex) {
