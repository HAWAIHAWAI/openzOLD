CREATE OR REPLACE FUNCTION zsdv_insertDatevExport(p_OrgID character varying,p_DateFrom character varying, p_DateTo character varying, p_user character varying,p_exuid character varying,  p_complete  character varying, p_allnew character varying )
...
BEGIN
      ...
      if (p_DateFrom is  null or p_DateTo is null) and p_allnew='N' then
         return 'Parameter Fehler. Bei Buchungssätzen müssen Beginn und Ende in einem Geschäftsjahr liegen.';
      end if;
      ...
      select count(*) into v_count from fact_acct where case when p_allnew='N' then dateacct between v_from and v_to else 1=1 END and 
                                 case when p_complete='N' then not exists (select 0 from ZSDV_Datev_ExportLines where ZSDV_Datev_ExportLines.fact_acct_group=fact_acct.fact_acct_group_id) ELSE 1=1 END;
      if v_count=0 then 
     ...
     -- EXPORT-Routine
      v_return:=zsdv_insertDatevExportLINES(p_OrgID,v_from,v_to,p_user,p_exuid, p_complete,p_allnew);
      return v_return;
...
CREATE OR REPLACE FUNCTION zsdv_insertDatevExportLINES(p_OrgID character varying,p_DateFrom timestamp without time zone, p_DateTo timestamp without time zone, p_user character varying,p_exuid character varying,  p_complete  character varying, p_allnew character varying )
  RETURNS character varying AS
$BODY$ 
DECLARE 
/***************************************************************************************************************************************************
...
v_belegfeld1  character varying:='';
v_belegfeld2  character varying:='';
...
BEGIN
      ...
      for v_cur1 in (select distinct fact_acct_group_id,ad_table_id from fact_acct where AD_ORG_ID=p_OrgID and 
                              case when p_allnew='N' then dateacct between p_DateFrom and p_DateTo else 1=1 END and
                              case when p_complete='N' then not exists (select 0 from ZSDV_Datev_ExportLines where ZSDV_Datev_ExportLines.fact_acct_group=fact_acct.fact_acct_group_id) ELSE 1=1 END)
      LOOP
          ...
          if v_count>0 and coalesce(v_onlydebcred,'N')='N' then 
              ...
              for v_cur2 in (select * from fact_acct where fact_acct_group_id=v_cur1.fact_acct_group_id and seqno!=999999 order by seqno)
              LOOP
                ...
                v_desc:=replace(substr(v_cur2.description,1,30),E'\r\n','');

                --v_satz:= substr(to_char(v_amt*100),1,length(to_char(v_amt*100))-3)||';;'||v_dracct||';;'||to_char(v_cur2.dateacct,'DDMMYYYY')||';'||v_cracct||';;;;'||v_desc||';;EUR;;;;;';
                if v_amt < 0 then
                      v_sollhaben:='H';
                      v_amt:=v_amt*(-1);
                else
                ...
                v_stramt:=zssi_strNumber(v_amt,'de_DE');
                --v_satz:= ';'||v_sollhaben||';'||v_stramt||';40;'||v_dracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';;0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                v_satz:= ';'||v_sollhaben||';'||v_stramt||';;'||v_dracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';;0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                v_line:=v_line+1;
                insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group, lineno,export_data )
          ...
          -- Table_id
              ...
              -- Tables sind Invoice, Manueller B-Stapel and Settlememnt ohne line 99999 (Das sind Abschreibungen mit Aufteilungen). Letztere werden über die line_id gefiltert, nicht über die Seq.-No.
                ...
                LOOP
                    ...
                    -- Datev Buchungsschlüssel bei Rechnungen und manuellem Buchungsstapel einsetzen.
                    -- Buchungsschlüssel bei Rechnungen 
                    if v_cur2.ad_table_id='318' and v_cur2.c_tax_id is null then
                        select t.name  into  v_belegfeld1
                        from c_tax t,c_invoiceline il where il.c_tax_id=t.c_tax_id and il.c_invoiceline_id=v_cur2.line_id
                        and t.rate!=0;
                    end if;
                    -- Buchungsschlüssel manueller Buchungsstapel
                    if v_cur2.ad_table_id='4AF9D81E51A04F2B987CD91AA9EE99F4' and v_cur2.c_tax_id is null then
                        select t.name,t.datevkeyvst,t.isnotaxable  into  v_belegfeld1,v_datevkeyvst,v_usedatevkey
                        from c_tax t,zsfi_macctline ml where ml.c_tax_id=t.c_tax_id and  ml.zsfi_macctline_id=v_cur2.record_id
                        and t.rate!=0;
                    end if;
                    if (v_cur2.seqno=10  or (v_lineid!='' and v_newgroup='Y')) then
                        ...
                        then
                            ...
                            v_stramt:=zssi_strNumber(v_sourceamt,'de_DE');
                            v_belegfeld1:=substr(replace(replace(replace(coalesce(v_belegfeld1,''),' ',''),'.',''),',',''),1,12);
                            v_belegfeld2:=substr(replace(replace(replace(coalesce(v_belegfeld2,''),' ',''),'.',''),',',''),1,12);
                            v_line:=v_line+1;
                            v_desc:=replace(substr(v_cur2.description,1,30),E'\r\n','');
                            v_satz:= ';'||v_source||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                            insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group,lineno, export_data )
                                values(get_uuid(),p_exuid,v_client, p_OrgID,p_user,p_user,v_cur2.dateacct,v_cur2.fact_acct_group_id,v_line,v_satz);
                            v_rcount:=v_rcount+1;
                            v_korrektur:='';
                    ...
                    --seqno=10
                        ...
                        if v_taxbooked='N' then
                            ...
                            --v_satz:= ';'||v_sollhaben||';'||v_stramt||';40;'||v_taxdracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_taxcracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;'; 
                            v_satz:= ';'||v_sollhaben||';'||v_stramt||';;'||v_taxdracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_taxcracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;'; 
                            insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group,lineno, export_data )
                                values(get_uuid(),p_exuid,v_client, p_OrgID,p_user,p_user,v_cur2.dateacct,v_cur2.fact_acct_group_id,v_line,v_satz);
                            v_rcount:=v_rcount+1;
                        end if; 
                        ...
                        --v_satz:= ';'||v_sollhaben||';'||v_stramt||';40;'||v_dracct||';;;'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                        v_satz:= ';'||v_sollhaben||';'||v_stramt||';'||v_korrektur||';'||v_dracct||';'||v_belegfeld1||';'||v_belegfeld2||';'||to_char(v_cur2.dateacct,'DDMM')||';'||v_cracct||';;;;0,00;'||v_desc||';'||v_uid||';0,00;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;';
                        v_line:=v_line+1;
                        insert into ZSDV_Datev_ExportLines (ZSDV_Datev_ExportLines_id,ZSDV_DATEV_EXPORT_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, dateacct,fact_acct_group,lineno, export_data )
                                values(get_uuid(),p_exuid,v_client, p_OrgID,p_user,p_user,v_cur2.dateacct,v_cur2.fact_acct_group_id,v_line,v_satz);
                        v_rcount:=v_rcount+1;
...
CREATE or replace FUNCTION zsfi_get_a_asset_id (
    p_ad_org_id     character varying,   -- ad_org_id.ad_org_id
    p_a_asset_value character varying    -- a_asset.value= '163' -- Betriebsarzt
  )
RETURNS
  CHARACTER VARYING  -- a_asset_id
AS $body_$
-- SELECT zsfi_get_a_asset_id('AE3637495E9E4EBFA7E766FE9B97893A', '101') AS zsfi_get_a_asset_id;  -- gueltig
-- SELECT zsfi_get_a_asset_id('AE3637495E9E4EBFA7E766FE9B97893A', '105') AS zsfi_get_a_asset_id; -- ungueltig
-- SELECT zsfi_get_a_asset_id('AE3637495E9E4EBFA7E766FE9B97893A', '1021') AS zsfi_get_a_asset_id; -- 1021 Katar
-- SELECT * FROM a_asset ass WHERE ass.value = '163'; -- Betriebsarzt
-- SELECT * FROM a_asset ass WHERE ass.value = '1021'; -- Katar
DECLARE
  v_result           character varying;

  v_org_ad_org_id    VARCHAR := '';
  v_org_isactive     VARCHAR := ''; -- N , Y
  v_ass_a_asset_id   VARCHAR := '';
  v_ass_isactive     VARCHAR := ''; -- N , Y
  v_ass_value        VARCHAR := '';
BEGIN
  SELECT
    org.ad_org_id, org.isactive,
    ass.a_asset_id, ass.isactive,
    ass.value
  INTO
    v_org_ad_org_id, v_org_isactive,
    v_ass_a_asset_id, v_ass_isactive,
    v_ass_value
  FROM a_asset ass, ad_org org
  WHERE 1=1
   AND org.ad_org_id = ass.ad_org_id
   AND ass.ad_org_id = p_ad_org_id
   AND ass.value = p_a_asset_value;

  IF ( (v_org_isactive = 'Y')
   AND (v_ass_isactive = 'Y') ) THEN
     IF ( LENGTH(v_ass_a_asset_id) > 0 ) THEN
       v_result :=  v_ass_a_asset_id;
     END IF;
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS(
    ...
    CREATE TABLE I_Primanota (
      ...
      i_05 VARCHAR(02),   -- 2
      i_06 VARCHAR(18),   -- 20111125103911150
      i_07 VARCHAR(27),   --
      i_08 VARCHAR(03),   -- "RE"
        ...
        -- SachKonto ggfls korrigieren
         IF NOT isempty(CUR_Lines.il_07) THEN
           IF (LENGTH(CUR_Lines.il_07) <= 3) THEN
             v_konto := LPAD(CUR_Lines.il_07, 4, '0000'); -- SKTO(4)
           ELSE
             v_konto := CUR_Lines.il_07; -- SKTO(4) / PKTO(P+5)
           END IF;
         END IF;
       ELSE
...
--            v_accounttype := 'A'; -- Asset (UV)
     ...
     -- Kostenstelle ggfls korrigieren
      IF NOT isempty(CUR_Lines.il_37) THEN
--      IF LENGTH(CUR_Lines.il_37 < 3) THEN
--         v_ksst_value := LPAD(CUR_Lines.il_37, 3, '000');
--       ELSE
--         v_ksst_value := CUR_Lines.il_37;
--       END IF;
         v_ksst_value := CUR_Lines.il_37;
         v_a_asset_id := (SELECT zsfi_get_a_asset_id(p_ad_org_id, v_ksst_value));
         IF isempty(v_a_asset_id) THEN
          -- INSERT INTO a_asset (
          --          a_asset_id,   ad_client_id,   ad_org_id, createdby, updatedby,        value,         name, description)
          -- VALUES ( get_uuid(), v_ad_client_id, p_ad_org_id,       '0',       '0', v_ksst_value, v_ksst_value, 'Datev-CSV-Import');
          -- k := k + 1;
          l := l + 1;       -- count a_asset found, not imported
          v_assetArray[l] := i || ' ' || v_ksst_value; -- ZeileNr + ' ' + KstStNr
         END IF;
      END IF;
...
DECLARE
  ...
  v_a_asset_id    VARCHAR;        -- il_37, Kostenstelle a_asset(a_asset_id)
...
BEGIN
  ...
  SELECT ad_client_id INTO v_ad_client_id
  FROM ad_org org WHERE org.ad_org_id = p_ad_org_id; -- 'AE3637495E9E4EBFA7E766FE9B97893A';
  ...
  BEGIN 
    FOR CUR_Lines IN (
      SELECT
        ...
        il_37,   -- AL: v_a_asset_id / a_asset.a_asset_id: Kostenstelle
        to_date(SUBSTRING(il_10, 1, 2) || '.' || SUBSTRING(il_10, 3, 2)||'.'|| to_char(v_16, 'YYYY'),'DD.MM.YYYY' ) as acctdate -- Buchungsdatum
    ...
    LOOP
      ...
      v_a_asset_id := (SELECT zsfi_get_a_asset_id(p_ad_org_id, CUR_Lines.il_37));
      ...
      IF (isempty(CUR_Lines.il_07)) THEN
        ...
        v_messArray[v_anzError] := v_message;
      ELSE
        IF (LENGTH(CUR_Lines.il_07) <= 3) THEN -- fehlende fuehrende '0' bei Sachkonto ergaenzen
          v_accountValue_07 := LPAD(CUR_Lines.il_07, 4, '0000');                      -- SKTO(3->4): '675' >> '0675'
        ...
        ELSE
          v_accountValue_07 := CUR_Lines.il_07;
      ...
      IF (isempty(CUR_Lines.il_08)) THEN
        ...
        v_messArray[v_anzError] := v_message;
      ELSE
        IF (LENGTH(CUR_Lines.il_08) <= 3) THEN -- fehlende fuehrende '0' bei Sachkonto ergaenzen
          v_accountValue_08 := LPAD(CUR_Lines.il_08, 4, '0000');                     -- SKTO(3->4): '675' >> '0675'
        ...
        ELSE
          v_accountValue_08 := CUR_Lines.il_08;
...
--      v_messArray[v_anzError] := v_message;
    ...
    --  ELSEIF ( (v_il_09) = '92') OR (v_il_09) = '95') ) THEN                      -- 9x Steuer bei Leistungsempfaenger : ohne Vorsteuer, mit 7%/19% Umsatzsteuer
     ...
     -- Pruefung Kostenstelle, wenn geliefert (NULL erlaubt)
      IF ( (CUR_Lines.il_37 IS NOT NULL) AND (LENGTH(CUR_Lines.il_37) > 0) ) THEN
        IF (v_a_asset_id IS NULL) THEN
          v_anzError := v_anzError + 1;
          v_message := 'Tabelle i_primanotaline, Zeile ' || i || ', Spalte il_37: Kostenstelle nicht im Kostenstellenstamm angelegt: ''' || CUR_Lines.il_37 || '''';
          v_messArray[v_anzError] := v_message;
          CONTINUE;
        END IF;
      END IF;
     ...
     -- Kontentyp Spalte il_07
      IF (LENGTH(CUR_Lines.il_07) <= 3) THEN -- fehlende fuehrende '0' bei Sachkonto ergaenzen
        v_accounttype_07 := zsfi_get_c_elementvalue(p_ad_org_id, LPAD(CUR_Lines.il_07, 4, '0000'), 'accounttype'); -- '675' >> '0675'
      ELSEIF( (LENGTH(CUR_Lines.il_07) = 5) AND (SUBSTRING(CUR_Lines.il_07, 1, 1) = '1') ) THEN
     ...
     -- Kontentyp Spalte il_08
      IF (LENGTH(CUR_Lines.il_08) <= 3) THEN -- fehlende fuehrende '0' bei Sachkonto ergaenzen
        v_accounttype_08 := zsfi_get_c_elementvalue(p_ad_org_id, LPAD(CUR_Lines.il_08, 4, '0000'), 'accounttype');   -- '675' >> '0675'
      ELSEIF( (LENGTH(CUR_Lines.il_08) = 5) AND (SUBSTRING(CUR_Lines.il_08, 1, 1) = '1') ) THEN
