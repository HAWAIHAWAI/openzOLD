  ...
  ELSIF (TG_OP = 'INSERT') THEN
    IF (new.C_Settlement_Generate_ID IS NOT NULL) THEN
      ...
      IF v_IsProcessed='Y' THEN
        RAISE EXCEPTION '%', 'The settlement is processed'; --OBTG:-20510--
      END IF;
    END IF;
  ...
  ELSIF (TG_OP = 'UPDATE') THEN
    ...
    IF (new.C_Settlement_Cancel_ID IS NOT NULL) THEN
     IF (old.C_Settlement_Cancel_ID<>new.C_Settlement_Cancel_ID) THEN
        RAISE EXCEPTION '%', 'The Debt/Payment is already in a Settlement Cancel'; --OBTG:-20512--
     END IF;
...
********************************************************************************************************************************/
 ...
 v_documentno_Dp_Management VARCHAR(50); 
...
END ; $BODY$
  LANGUAGE 'plpgsql' VOLATILE
  COST 100;
ALTER FUNCTION c_bankstatement_post(character varying) OWNER TO tad;
...
BEGIN
  ...
  IF(TG_OP = 'UPDATE' OR TG_OP = 'INSERT') THEN
    select isreceipt into v_isreceip from c_debt_payment where c_debt_payment_id=new.c_debt_payment_id;
    if v_isreceip='Y' and new.sepaexportenabled='Y' then
       raise exception '%','@sepaexportonlywithpayments@';
    end if;
  END IF;
  IF(TG_OP = 'INSERT') THEN
     select isreceipt into v_isreceip from c_debt_payment where c_debt_payment_id=new.c_debt_payment_id;
     if v_isreceip='N' then
...
CREATE or replace FUNCTION zssi_getOwnBankaccountFromDebtPayment( p_debt_payment_id character varying) RETURNS character varying
AS $_$
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
compliance with the License. You may obtain a copy of the License at http://www.mozilla.org/MPL/MPL-1.1.html
...
BEGIN
      select c_bankaccount_id,ad_org_id into v_acct,v_org from c_debt_payment where c_debt_payment_id=p_debt_payment_id;
      if v_acct is null then 
          select  bs.c_bankaccount_id into v_acct from c_bankstatement bs,c_bankstatementline bsl where bsl.c_bankstatement_id=bs.c_bankstatement_id and bsl.c_debt_payment_id=p_debt_payment_id;
      end if;
      if v_acct is null then 
          select c_bankaccount_id into v_acct from c_bankaccount where ad_org_id in ('0',v_org) and isdefault='Y' and isactive='Y';
      end if;
RETURN v_acct;
END;
$_$
