...
public class ConfigureTextarea {
  ...
  private static StringBuilder doConfigureAll(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String fieldname, int leadingemptycols,int colstotal,int maxlength,boolean required,boolean readonly,String callout,String currentvalue,String labeltext, boolean isGrid, String jssettings, String action, String title, String style,String elementId) throws Exception{
    ...
    if (style.isEmpty())
      style="TextArea_FiveCells_width TextArea_Medium_height";
    for (int i = 0; i < leadingemptycols; i++) {
      retval=retval.append("<td></td>");
    }
    if (! isGrid)
      retval=retval.append(ConfigureLabel.doConfigure(servlet,vars,fieldname,"Label_ContentCell",labeltext,elementId));
    Object template =  servlet.getServletContext().getAttribute("textareaTEMPLATE");
    if (template==null) {
      template = new String(FileUtils.readFile("Textarea.xml", directory + "/src-loc/design/org/openz/view/templates/"));
      servlet.getServletContext().setAttribute("textareaTEMPLATE", template);
    }
    ...
    Replace.replace(retval, "@CLASS@","dojoValidateValid @REQUIRED@  TextArea_Medium_height");
    ...
    Replace.replace(retval, "@TEXTAREACONTENT@", currentvalue);
