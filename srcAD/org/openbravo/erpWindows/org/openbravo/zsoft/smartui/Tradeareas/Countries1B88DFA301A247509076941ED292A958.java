...
*/
  private Countries1B88DFA301A247509076941ED292A958Data getEditVariables(Connection con, VariablesSecureApp vars, String strPZssi_Tradearea_ID) throws IOException,ServletException {
    ...
    try {
    data.zssiTradeareaCountryId = vars.getRequestGlobalVariable("inpzssiTradeareaCountryId", windowId + "|Zssi_Tradearea_Country_ID");     data.adClientId = vars.getRequestGlobalVariable("inpadClientId", windowId + "|AD_Client_ID");     data.adClientIdr = vars.getStringParameter("inpadClientId_R");     data.adOrgId = vars.getRequestGlobalVariable("inpadOrgId", windowId + "|AD_Org_ID");     data.adOrgIdr = vars.getStringParameter("inpadOrgId_R");     data.zssiTradeareaId = vars.getStringParameter("inpzssiTradeareaId");     data.cCountryId = vars.getStringParameter("inpcCountryId");     data.cCountryIdr = vars.getStringParameter("inpcCountryId_R");     data.isactive = vars.getStringParameter("inpisactive", "N"); 
   ...
   private Countries1B88DFA301A247509076941ED292A958Data[] getRelationData(Countries1B88DFA301A247509076941ED292A958Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].zssiTradeareaCountryId = FormatUtilities.truncate(data[i].zssiTradeareaCountryId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].zssiTradeareaId = FormatUtilities.truncate(data[i].zssiTradeareaId, 32);        data[i].cCountryId = FormatUtilities.truncate(data[i].cCountryId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strZssi_Tradearea_Country_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|Zssi_Tradearea_Country_ID", data[0].getField("zssiTradeareaCountryId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));
    }
