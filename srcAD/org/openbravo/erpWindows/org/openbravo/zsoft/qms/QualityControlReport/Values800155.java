...
vars.getRequestGlobalVariable("inpParamMA_Measure_Time_ID", tabId + "|paramMA_Measure_Time_ID");
vars.getRequestGlobalVariable("inpParamMA_Ccp_ID", tabId + "|paramMA_Ccp_ID");
...
/*    } else if (vars.commandIn("DELETE_RELATION")) {
      ...
      String strMA_Measure_Values_ID = vars.getRequiredInStringParameter("inpmaMeasureValuesId");
      String message = deleteRelation(vars, strMA_Measure_Values_ID, strPMA_Measure_Time_ID);
      if (!message.equals("")) {
...
/*
  String deleteRelation(VariablesSecureApp vars, String strMA_Measure_Values_ID, String strPMA_Measure_Time_ID) throws IOException, ServletException {
    log4j.debug("Deleting records");
    Connection conn = this.getTransactionConnection();
    try {
      if (strMA_Measure_Values_ID.startsWith("(")) strMA_Measure_Values_ID = strMA_Measure_Values_ID.substring(1, strMA_Measure_Values_ID.length()-1);
      if (!strMA_Measure_Values_ID.equals("")) {
        strMA_Measure_Values_ID = Replace.replace(strMA_Measure_Values_ID, "'", "");
        StringTokenizer st = new StringTokenizer(strMA_Measure_Values_ID, ",", false);
        while (st.hasMoreTokens()) {
          String strKey = st.nextToken();
          if (Values800155Data.deleteTransactional(conn, this, strKey, strPMA_Measure_Time_ID)==0) {
...
String strParamMA_Ccp_ID = vars.getSessionValue(tabId + "|paramMA_Ccp_ID");
    ...
    if (!(strParamMA_Measure_Time_ID.equals("") && strParamMA_Ccp_ID.equals(""))) hasSearchCondition=true;
    String strOffset = "0";
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPMA_Measure_Time_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = SequenceIdData.getUUID();
          ...
          CrudOperations.UpdateCustomFields(tabId, data.maMeasureValuesId, vars, this);
        } catch(Exception ex) {
