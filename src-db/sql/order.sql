  ...
  BEGIN --BODY
    ...
    */
    ...
    SELECT Processing, Processed, DocAction, DocStatus,
      ...
      C_BPartner_ID, M_PriceList_ID, invoicerule,issotrx
    INTO v_IsProcessing, v_IsProcessed, v_DocAction, v_DocStatus,
      ...
      v_CBPartner_ID, v_PriceList_ID, v_invoicerule,v_issotrx
    FROM C_ORDER
      ...
      IF(v_DocAction='VO') THEN
        -- SZ: Checks for Order Voiding:
                ...
                if v_DocStatus='CO' and v_DocType_ID in ('5EED1EFB8BDD4C0491ECCFD7395DA446','9D7785AF1F0D4C51A0B1DF45F5DC9EE5') then
                    ...
                    for v_cur in (select * from c_orderline where c_order_id = v_Record_ID) 
                    LOOP
                        update c_orderline set calloffqty=coalesce(calloffqty,0) - (v_cur.qtyordered+v_cur.qtyinvoiced),deliverycomplete='N' where c_orderline_id=v_cur.orderlineselfjoin;
                    END LOOP;
                end if;
...
*************************************************************************/
    ...
    If ad_get_docbasetype(v_DocType_ID) not in ('NON','POREQUESTOFFER','SALESOFFER') then
            ...
            */
            ...
            /**
            * SZ Updating Product-PO - Moved from Acct-Server to here
            */    
            if v_issotrx='N' then
            ...
            *************************************************************************/
            IF(v_DocSubTypeSO IN('WI', 'WP', 'WR')) THEN
                  ...
                  */
                  ...
                  insert into c_generateminoutmanual(C_GENERATEMINOUTMANUAL_ID, C_ORDERLINE_ID, C_ORDER_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, QTY, M_LOCATOR_ID,pinstance_id)
                         select get_uuid(),ol.C_ORDERLINE_ID, ol.C_ORDER_ID, ol.AD_CLIENT_ID, ol.AD_ORG_ID, ol.CREATEDBY, ol.UPDATEDBY, ol.qtyordered, 
                                m_gettransactionlocator(ol.m_product_id,o.m_warehouse_id,o.issotrx,ol.qtyordered),v_nexpinstance
                  ...
                  IF(v_DocSubTypeSO IN('WI', 'WR')) THEN
                    ...
                    */
                    ...
                    insert into C_GENERATEINVOICEMANUAL (C_GENERATEINVOICEMANUAL_ID, C_ORDERLINE_ID, C_ORDER_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, QTY, PRICE, DESCRIPTION,LINEAMT,pinstance_id)
                           select get_uuid(),ol.C_ORDERLINE_ID, ol.C_ORDER_ID, ol.AD_CLIENT_ID, ol.AD_ORG_ID, ol.CREATEDBY, ol.UPDATEDBY, ol.qtyordered, ol.priceactual, ol.DESCRIPTION,ol.linenetamt,v_nexpinstance
                           from c_orderline ol where ol.C_Order_ID=v_Record_ID;
...
ALTER FUNCTION public.c_order_post1(p_pinstance_id character varying, p_order_id character varying) OWNER TO tad;
         ...
         IF  v_cur_line.m_product_id is not null then
             ...
             if v_iscalcenabled='Y' then 
                ...
                if v_dummy>0 then
                    ...
                    update m_product_po set UPDATED=now(),UPDATEDBY=new.UPDATEDBY,ISCURRENTVENDOR='Y',C_UOM_ID=v_uom, C_CURRENCY_ID=new.C_CURRENCY_ID,PRICELIST=v_cur_line.pricelist,
...
BEGIN 
    ...
    LOOP
        ...
        INSERT INTO C_Order
              ...
              yearly_month,weekly_day,monthly_day,quarterly_month,isinvoiceafterfirstcycle,internalnote,c_projecttask_id)
            VALUES
              (v_COrder_ID, v_Client_ID, v_Org_ID,v_cur.isactive,p_User, p_User,
              v_cur.issotrx, v_DocumentNo,  'DR', 'CO','N',
              v_CDocTypeID, v_CDocTypeID, 
              ...
              v_cur.yearly_month,v_cur.weekly_day,v_cur.monthly_day,v_cur.quarterly_month,v_cur.isinvoiceafterfirstcycle,v_cur.internalnote,v_cur.c_projecttask_id);
    END LOOP;
    v_dummy:= c_copyorderlines(p_Record_ID,v_COrder_ID,p_User);
...
BEGIN 
    ...
    LOOP
         INSERT INTO C_OrderLine
            ...
            PRICELIST, PRICEACTUAL, PRICELIMIT,
            PRICESTD,  DISCOUNT,
            C_TAX_ID,quantityorder,m_product_uom_id,c_project_id,a_asset_id,isonetimeposition)
          VALUES
            (p_destorder_id,get_uuid(),v_Client_ID, v_Org_ID,p_user_id, p_User_id,
             ...
             v_cur.c_project_id,v_cur.a_asset_id,case v_copy_onetimepositions when 'Y' then v_cur.isonetimeposition else 'N' end);
         v_i:=v_i+1;
    END LOOP;
    -- Payment scheduling
...
BEGIN 
    select ad_client_id,ad_org_id into  v_client_id,v_org_id from c_order where c_order_id=p_sourceorder_id;
    select c_bpartner_id into v_partner  from c_order where c_order_id=p_destorder_id ;
    --raise notice '%','copyorderlines - Source:'||coalesce(p_sourceorder_id,'0');
    --raise notice '%','copyorderlines - Dest:'||coalesce(p_destorder_id,'0');
    -- Copy the lines from the given Order
    for v_cur in (
    ...
    LOOP
         ...
         INSERT INTO C_OrderLine
            ...
            PRICELIST, PRICEACTUAL, PRICELIMIT,
            PRICESTD,  DISCOUNT,
            C_TAX_ID,quantityorder,m_product_uom_id,c_project_id,a_asset_id,isonetimeposition, ref_orderline_id)
          VALUES
            (p_destorder_id,get_uuid(),v_Client_ID, v_Org_ID,p_user_id, p_User_id,
             v_cur.ISACTIVE,v_cur.LINE, v_cur.C_BPARTNER_ID, v_cur.C_BPARTNER_LOCATION_ID,
             v_cur.DATEORDERED, v_cur.DATEPROMISED, v_cur.DESCRIPTION, v_cur.M_PRODUCT_ID,
             v_cur.M_WAREHOUSE_ID, v_cur.C_UOM_ID, v_cur.QTYORDERED, v_cur.C_CURRENCY_ID,
             v_cur.PRICELIST, v_price, v_cur.PRICELIMIT,
             v_cur.PRICESTD,  v_cur.DISCOUNT,
             v_cur.C_TAX_ID,v_cur.quantityorder,v_cur.m_product_uom_id,
             v_cur.c_project_id,v_cur.a_asset_id,v_cur.isonetimeposition, v_cur.c_orderline_id);
         v_i:=v_i+1;
    END LOOP;
    -- Payment scheduling
    for v_cur in (select * from c_order_paymentschedule where c_order_id=p_sourceorder_id)
...
BEGIN 
    select ad_client_id,ad_org_id into  v_client_id,v_org_id from c_order where c_order_id=p_sourceorder_id;
    -- One Time Position Flag can b3e copied only into Subscriptions and Subscription Proposals.
    select case c_doctype_id when '7DE8D4B1B8824D36974E8064BBED5095' then 'Y' when 'ABE2033C7A74499A9750346A83DE3307' then 'Y' else 'N' end,c_bpartner_id,c_project_id,datepromised,scheddeliverydate,dateordered into v_copy_onetimepositions,v_bpartner,v_project,v_datepromised,v_scheddeliverydate,v_dateordered
    ...
    LOOP
         INSERT INTO C_OrderLine
            ...
            PRICELIST, PRICEACTUAL, PRICELIMIT,
            PRICESTD,  DISCOUNT,
            C_TAX_ID,quantityorder,m_product_uom_id,isonetimeposition,c_project_id,datepromised,scheddeliverydate)
          VALUES
            (p_destorder_id,get_uuid(),v_Client_ID, v_Org_ID,p_user_id, p_User_id,
             v_cur.ISACTIVE,v_cur.LINE, v_bpartner, v_cur.C_BPARTNER_LOCATION_ID,
             v_dateordered,  v_cur.DESCRIPTION, v_cur.M_PRODUCT_ID,
             v_cur.M_WAREHOUSE_ID, v_cur.C_UOM_ID, v_cur.QTYORDERED, v_cur.C_CURRENCY_ID,
             v_cur.PRICELIST, v_cur.PRICEACTUAL, v_cur.PRICELIMIT,
             v_cur.PRICESTD,  v_cur.DISCOUNT,
             v_cur.C_TAX_ID,v_cur.quantityorder,v_cur.m_product_uom_id,
             case v_copy_onetimepositions when 'Y' then v_cur.isonetimeposition else 'N' end,v_project,v_datepromised,v_scheddeliverydate);
         v_i:=v_i+1;
    END LOOP;
    -- Payment scheduling
    for v_cur in (select * from c_order_paymentschedule where c_order_id=p_sourceorder_id)
...
loop
	...
	loop
		...
		if v_orderline.isonetimeposition = 'N' -- Subscription position, always insert
			...
			(select count(*) from c_orderline where ref_orderline_id=v_orderline.ref_orderline_id and
			(select docstatus from c_order where c_order_id = (select distinct c_order_id from c_orderline where ref_orderline_id=v_orderline.ref_orderline_id limit 1)) in ('CO'))=0
			then 
