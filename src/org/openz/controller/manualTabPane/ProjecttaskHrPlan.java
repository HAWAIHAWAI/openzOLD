...
public class ProjecttaskHrPlan implements ManualTabPane{
  ...
  public String getFormEdit(HttpSecureAppServlet servlet,VariablesSecureApp vars,FieldProvider data,WindowTabs tabs) throws Exception{
    ...
    try {
      ...
      if (strCommand.equals("SAVE")) {
        ...
        String strSeqno,strHRPId,strCategory,strQty,strPlannedUser,strovertimehours,strnighthours,strsaturday,strsunday,strholiday,strspecialtime1,strspecialtime2,strtriggeramt,strdatefrom,strdateto;
        for (int i = 0; i < retval.size(); i++) {
          ...
          if (!strCategory.isEmpty()) {
            ...
            if (ProjecttaskHRPlanData.isExistingID(servlet, strHRPId).equals("0"))
              ProjecttaskHRPlanData.insert(conn, servlet, strHRPId,strOrgid, strProjecttaskid,strUser, strCategory, strQty, strPlannedUser,
                  strovertimehours,strnighthours,strsaturday,strsunday,strholiday,strspecialtime1,strspecialtime2,strtriggeramt,strdatefrom,strDateFormat,strdateto);
            else
              ProjecttaskHRPlanData.update(conn, servlet,strUser, strCategory, strQty, strPlannedUser,
                  strovertimehours,strnighthours,strsaturday,strsunday,strholiday,strspecialtime1,strspecialtime2,strtriggeramt,strdatefrom,strDateFormat,strdateto,strHRPId);
            if (msg.isEmpty())
      ...
      if (strCommand.equals("PICKLIST")) {
        ...
        for (int i = 0; i < retval.size(); i++) {
          ...
          if (ProjecttaskHRPlanData.isExistingEmployee(servlet, strProjecttaskid,strPlannedUser).equals("0") && !strCategory.isEmpty()) {
            ProjecttaskHRPlanData.insert(conn, servlet, strHRPId,strOrgid, strProjecttaskid,strUser, strCategory, strQty, strPlannedUser,
                strovertimehours,strnighthours,strsaturday,strsunday,strholiday,strspecialtime1,strspecialtime2,strtriggeramt,null,strDateFormat,null);
            if (msg.isEmpty())
              msg=Utility.messageBD(servlet, "HRPlanCreatedSucessfully",vars.getLanguage());
          }
      ...
      if (strCommand.equals("DELETE")) {
        ...
        for (int i = 0; i < retval.size(); i++) {
          strHRPId=retval.elementAt(i);
          ProjecttaskHRPlanData.delete(conn, servlet, strHRPId);
          msg=Utility.messageBD(servlet, "HRPlanCreatedSucessfully",vars.getLanguage());
        }
        servlet.releaseCommitConnection(conn);
      }
