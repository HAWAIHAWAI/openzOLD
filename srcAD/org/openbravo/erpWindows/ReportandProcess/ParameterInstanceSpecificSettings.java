...
vars.getRequestGlobalVariable("inpParamAD_Process_Para_ID", tabId + "|paramAD_Process_Para_ID");
...
/*    } else if (vars.commandIn("DELETE_RELATION")) {
      ...
      String strAD_Process_Para_Instance_ID = vars.getRequiredInStringParameter("inpadProcessParaInstanceId");
      String message = deleteRelation(vars, strAD_Process_Para_Instance_ID, strPAD_Process_Para_ID);
      if (!message.equals("")) {
...
/*
  String deleteRelation(VariablesSecureApp vars, String strAD_Process_Para_Instance_ID, String strPAD_Process_Para_ID) throws IOException, ServletException {
    log4j.debug("Deleting records");
    Connection conn = this.getTransactionConnection();
    try {
      if (strAD_Process_Para_Instance_ID.startsWith("(")) strAD_Process_Para_Instance_ID = strAD_Process_Para_Instance_ID.substring(1, strAD_Process_Para_Instance_ID.length()-1);
      if (!strAD_Process_Para_Instance_ID.equals("")) {
        strAD_Process_Para_Instance_ID = Replace.replace(strAD_Process_Para_Instance_ID, "'", "");
        StringTokenizer st = new StringTokenizer(strAD_Process_Para_Instance_ID, ",", false);
        while (st.hasMoreTokens()) {
          String strKey = st.nextToken();
          if (ParameterInstanceSpecificSettingsData.deleteTransactional(conn, this, strKey, strPAD_Process_Para_ID)==0) {
...
*/
  ...
  private void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strPAD_Process_Para_ID, String strAD_Process_Para_Instance_ID, TableSQLData tableSQL)
    ...
    if (!(strParamAD_Process_Para_ID.equals(""))) hasSearchCondition=true;
    String strOffset = "0";
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPAD_Process_Para_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = SequenceIdData.getUUID();
          ...
          CrudOperations.UpdateCustomFields(tabId, data.adProcessParaInstanceId, vars, this);
        } catch(Exception ex) {
