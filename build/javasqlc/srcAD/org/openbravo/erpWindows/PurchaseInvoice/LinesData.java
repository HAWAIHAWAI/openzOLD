 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Client_ID = (?) , AD_Org_ID = (?) , C_Invoice_ID = (?) , C_OrderLine_ID = (?) , M_InOutLine_ID = (?) , Reinvoicedby_ID = (?) , Line = TO_NUMBER(?) , M_Product_ID = (?) , C_Charge_ID = (?) , Description = (?) , QuantityOrder = TO_NUMBER(?) , M_Product_Uom_Id = (?) , QtyInvoiced = TO_NUMBER(?) , C_UOM_ID = (?) , Isgrossprice = (?) , PriceActual = TO_NUMBER(?) , PriceList = TO_NUMBER(?) , C_Tax_ID = (?) , A_Asset_ID = (?) , C_Project_ID = (?) , C_Projecttask_ID = (?) , Linegrossamt = TO_NUMBER(?) , LineNetAmt = TO_NUMBER(?) , PriceStd = TO_NUMBER(?) , IsDescription = (?) , C_Projectphase_ID = (?) , M_AttributeSetInstance_ID = (?) , ChargeAmt = TO_NUMBER(?) , PriceLimit = TO_NUMBER(?) , S_ResourceAssignment_ID = (?) , TaxAmt = TO_NUMBER(?) , IsActive = (?) , Linetaxamt = TO_NUMBER(?) , C_InvoiceLine_ID = (?) , Excludeforwithholding = (?) , M_Offer_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_InvoiceLine.C_InvoiceLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Client_ID, AD_Org_ID, C_Invoice_ID, C_OrderLine_ID, M_InOutLine_ID, Reinvoicedby_ID, Line, M_Product_ID, C_Charge_ID, Description, QuantityOrder, M_Product_Uom_Id, QtyInvoiced, C_UOM_ID, Isgrossprice, PriceActual, PriceList, C_Tax_ID, A_Asset_ID, C_Project_ID, C_Projecttask_ID, Linegrossamt, LineNetAmt, S_ResourceAssignment_ID, TaxAmt, M_AttributeSetInstance_ID, Linetaxamt, ChargeAmt, C_InvoiceLine_ID, PriceLimit, M_Offer_ID, PriceStd, IsDescription, C_Projectphase_ID, IsActive, Excludeforwithholding, created, createdby, updated, updatedBy)" +
