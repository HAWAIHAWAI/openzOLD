...
public class BusinessPartner extends HttpSecureAppServlet {
  ...
  private static final String[] colNames = { "value", "name", "pc", "city","address1","address2", "contact", "phone", "c_bpartner_id",
...
//"so_creditavailable","so_creditused","income",
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strNameValue = vars.getRequestGlobalVariable("inpKeyValue", "BusinessPartner.key");//key 2x<> name 2x (DH 4/12 Sufu mit Name)
      final String strIDValue = vars.getStringParameter("inpIDValue");
      final String strKeyValue = vars.getGlobalVariable("inpName", "BusinessPartner.name", "");//inpkey key<>inpname name  (DH 4/12 Sufu mit Name)
      vars.setSessionValue("BusinessPartner.adorgid", vars.getStringParameter("inpAD_Org_ID", ""));
      if (!strIDValue.equals("")) {
        final String strNameAux = BusinessPartnerData.existsActual(this, strKeyValue, strIDValue);//key<>name (DH 4/12 Sufu mit Name)
        if (!strNameAux.equals(""))
          strNameValue = strNameAux;
      }

      vars.removeSessionValue("BusinessPartner.name");//name<>key (DH 4/12 Sufu mit Name)
      ...
      if (strIsSOTrxTab.equals(""))
        strBpartner = BusinessPartnerData.issotrxwindow(this, strWindowId);
      ...
      printPage(response, vars, strKeyValue, "%", strSelected, "paramName"); //Name<>key (DH 4/12 Sufu mit Name)
    } else if (vars.commandIn("KEY")) {
      final String strWindowId = vars.getStringParameter("WindowID");
      final String strIsSOTrxTab = vars.getStringParameter("inpisSOTrxTab");
      String strKeyValue = vars.getRequestGlobalVariable("inpNameValue", "BusinessPartner.name");//name<>key (DH 4/12 Sufu mit Name)
      final String strIDValue = vars.getStringParameter("inpIDValue");
      ...
      vars.removeSessionValue("BusinessPartner.key");//key<>name (DH 4/12 Sufu mit Name)
      if (!strKeyValue.equals(""))
        vars.setSessionValue("BusinessPartner.name", strKeyValue + "%"); //name<> key (DH 4/12 Sufu mit Name)
      String strBpartner = strIsSOTrxTab;
      if (strIsSOTrxTab.equals(""))
        strBpartner = Utility.getContext(this, vars, "isSOTrx", strWindowId);
      String strSelected = "all";
      ...
      final BusinessPartnerData[] data = BusinessPartnerData.selectKey(this, 
          (strSelected.equals("customer") ? "xclients" : ""),
          (strSelected.equals("vendor") ? "xvendors" : ""),
      ...
      if (data != null && data.length == 1) {
        ...
        printPage(response, vars, strKeyValue + "%", "", strSelected, "paramName"); //paramKey <> paramName (DH 4/12 Sufu mit Name)
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strKeyValue,
    ...
    if (strKeyValue.equals("") && strNameValue.equals("")) {
      xmlDocument.setParameter("name", "%"); //name<> key (DH 4/12 Sufu mit Name)
    } else {
      xmlDocument.setParameter("name", strKeyValue); //name<>key (DH 4/12 Sufu mit Name)
    }
    xmlDocument.setParameter("directory", "var baseDirectory = \"" + strReplaceWith + "/\";\n");
    xmlDocument.setParameter("language", "defaultLang=\"" + vars.getLanguage() + "\";");
    xmlDocument.setParameter("theme", vars.getTheme());
    xmlDocument.setParameter("key", strNameValue); //key<>name (DH 4/12 Sufu mit Name)
  ...
  private SQLReturnObject[] getHeaders(VariablesSecureApp vars) {
    ...
    for (int i = 0; i < colNames.length; i++) {
      ...
      String tm = colNames[i].toUpperCase();
    ...
    if (headers != null) {
      try {
        ...
        if (strNewFilter.equals("1") || strNewFilter.equals("")) {
          String rownum = "0",  pgLimit = null;
          
         pgLimit = TableSQLData.maxRowsPerGridPage + " OFFSET " + offset;
         
          strNumRows = BusinessPartnerData.countRows(this, rownum, 
              (strBpartners.equals("customer") && strContact.isEmpty() ? "xclients" : ""),
              (strBpartners.equals("vendor") && strContact.isEmpty() ? "xvendors" : ""),
              Utility.getContext(this, vars,"#User_Client", "BusinessPartner"), 
              Utility.getSelectorOrgs(this, vars, strOrg),
              strKey, strName, strContact, strZIP, 
