...
if p_workstep is not null then
    ...
    If v_forcematerialscan='Y' then
       out_messagetext:='@zssm_ManualStockTransactionRequred@';
    else
...
BEGIN
    ...
    --
    -- Some Checks before...
    select qty,qtyproduced into v_orig_qty,v_orig_prod from c_projecttask where c_projecttask_id=v_Record_ID;
    if v_orig_qty=0 then v_orig_qty:=1; end if;
    select count(*) into v_Count from zspm_projecttaskbom where c_projecttask_id=v_Record_ID and (quantity/v_orig_qty)*(v_orig_prod+v_qty)>qtyreceived;
    if v_count>0 then
    ...
    */
           ...
           if v_isserial=true then
                v_Message:=v_Message||'@zssm_MaterialReturnOnlyAfterSerialRegistration@';
                 ...
                 insert into M_INTERNAL_CONSUMPTION(M_INTERNAL_CONSUMPTION_ID, AD_CLIENT_ID, AD_ORG_ID,  CREATED, CREATEDBY, UPDATED, UPDATEDBY,
                           ...
                           values(v_uid,v_client,v_Org,NOW(), v_User, NOW(),v_User,
                                  'Production-Process','Generated by Production-> Return Material',now(),now(),v_project, v_Record_ID,'D+',v_DocumentNo);
                 for v_cur in (select case when v_isassemble='N' then qtyreceived else qtyreceived-(qtyreceived / v_qty_orig * (v_qty+v_qtyprocessed)) end as qty,m_product_id,issuing_locator,zspm_projecttaskbom_id  from zspm_projecttaskbom where c_projecttask_id=v_Record_ID)
                 LOOP
                    -- uom
                    select c_uom_id,isserialtracking,isbatchtracking into v_uom,v_serial,v_batch from m_product where m_product_id=v_cur.m_product_id;
                    v_lineUUId:=get_uuid();
    ...
    if v_isserial=false then
          update c_projecttask set iscomplete='Y',ended=now() where c_projecttask_id=v_Record_ID;
    end if;
    ---- <<FINISH_PROCESS>>
    --  Update AD_PInstance
    RAISE NOTICE '%','Updating PInstance - Finished ' || v_Message ;
