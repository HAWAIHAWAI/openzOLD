...
public class PdcMaterialConsumption extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    try {
      ...
      String UpperGridADName = "PdcMaterialConsumptionUpperGrid";
      String ButtonADName = "PdcMaterialConsumptionButtons";
      String LowerGridADName = "PdcMaterialConsumptionLowerGrid";
      ...
      String SaveADName = "pdcmaterialconsumptionsavebutton";
      String DoneADName = "pdcmaterialconsumptiondonebutton";
      String CancelADName = "pdcmaterialconsumptioncancelbutton";
    ...
    //setting History
      ...
      if (vars.commandIn("SAVE_NEW_NEW")) {
        if (!vars.getStringParameter("inp" + BarcodeADName).isEmpty()) {
          ...
          if (bctype.equals("EMPLOYEE")) {
            ...
            String qty=PdcMaterialConsumptionData.getQty(this, GlobalConsumptionID, GlobalWorkstepID, bcid, locator);
            if (qty.isEmpty())
              qty="1";
      ...
      if (vars.commandIn("ALLPOSITIONS")||BcCommand.equals("ALLPOSITIONS")){
        if (getLocalSessionVariable(vars, WorkstepIDADName).equals("")||
          ...
          upperGridData = PdcMaterialConsumptionData.selectupper(this, vars.getLanguage(),GlobalConsumptionID, getLocalSessionVariable(vars, WorkstepIDADName)); 
          for (int i=0;i<upperGridData.length;i++){
            if (upperGridData[i].getField("pdcmaterialconsumptionlocator").isEmpty()) {
              vars.setSessionValue("PDCSTATUS","ERROR");
              vars.setSessionValue("PDCSTATUSTEXT",Utility.messageBD(this, "pdc_MustSetProductQtyAndLocator",vars.getLanguage()));
            } else {
              String qty=PdcMaterialConsumptionData.getQty(this, GlobalConsumptionID, GlobalWorkstepID, upperGridData[i].getField("m_product_id"), upperGridData[i].getField("m_locator_id"));
              if (! qty.isEmpty()) {
        ...
        if (getLocalSessionVariable(vars, LocatorIDADName).equals("")||
          ...
          if (vars.commandIn("NEXT")||BcCommand.equals("NEXT")) {
           // Check if Value Updates a line or deletes a line
            ...
            // Qty > 0 and new line
            if (sameline.equals("") && new BigDecimal(getLocalSessionVariable(vars, QuantityADName)).compareTo(BigDecimal.ZERO)==1) {
              PdcCommonData.insertMaterialLine(conn, this, vars.getClient(), vars.getOrg(), 
      ...
      if (getLocalSessionVariable(vars, UserIDADName) == "") {
        ...
        InfobarText = Utility.messageBD(this, "pdc_ScanNCCA",vars.getLanguage());
      ...
      upperGridData = PdcMaterialConsumptionData.selectupper(this, vars.getLanguage(),GlobalConsumptionID, getLocalSessionVariable(vars, WorkstepIDADName));   // Load upper grid date with language for translation
      ...
      strToolbar = FormhelperData.getFormToolbar(this, this.getClass().getName());                                                    // Load toolbar ID
      strSkeleton = ConfigureFrameWindow.doConfigure(this, vars, "UserID",null, "Material Consumption", strToolbar, "NONE", tabs);   // Generate skeleton
