...
public class FormDisplayLogic {
      ...
      if (visiblelogic.equals("HIDDEN")||visiblelogic.equals("HIDDENNUMERIC")) {
        if (script!=null && addscriptlogic) {
          if (visiblelogic.equals("HIDDENNUMERIC"))
            script.addHiddenfield("inp" + Sqlc.TransformaNombreColumna(fieldname.toLowerCase()), FormatUtils.formatNumber(value,vars,"qtyEdition"));
    ...
    private static String evaluateLogicTrue(HttpSecureAppServlet servlet,VariablesSecureApp vars,Scripthelper script,String logic,String adFieldId) throws Exception {
      /*
      ...
      */
      ...
      if (logic.startsWith("@SQL=")){
        ...
        retval="if ('" + getSQL(sql,servlet).toUpperCase() + "'=='TRUE'){\n";
        return retval;
      }
      while (true) {
    ...
    private static String tokenizeSQL(HttpSecureAppServlet servlet,VariablesSecureApp vars,String sql,String fieldid) throws Exception {
      /*
     ...
     */
      ...
      while (true) {
        ...
        if (FormDisplayLogicData.isFieldInForm(servlet, fieldid, token).equals("Y"))
          if (datatype.equals("NUMERIC"))
            tokenvalue=vars.getNumericParameter("inp" + token);
          else
            tokenvalue=vars.getStringParameter("inp" + token);
        else{
    ...
    private static String getSQL(String sql, ConnectionProvider conn) throws Exception {
        String columname="";
        String testsql=sql.toLowerCase();
        if(testsql.contains(" as "))
          columname=sql.substring(testsql.indexOf(" as ")+3,testsql.indexOf(" from ")).trim();
        else
          columname=sql.substring(testsql.indexOf("select ")+6 ,testsql.indexOf(" from ")).trim();
        ...
        String retval = "";
        if (srs.first())
          retval = srs.getString(columname);
        dbcon.close();
        if (retval==null)
          retval = "";
        return retval;
