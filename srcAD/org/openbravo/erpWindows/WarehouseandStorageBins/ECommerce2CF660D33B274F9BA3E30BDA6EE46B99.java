...
*/
  private ECommerce2CF660D33B274F9BA3E30BDA6EE46B99Data getEditVariables(Connection con, VariablesSecureApp vars, String strPM_Warehouse_ID) throws IOException,ServletException {
   ...
   private ECommerce2CF660D33B274F9BA3E30BDA6EE46B99Data[] getRelationData(ECommerce2CF660D33B274F9BA3E30BDA6EE46B99Data[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mWarehouseId = FormatUtilities.truncate(data[i].mWarehouseId, 32);        data[i].zseWarehouseShopId = FormatUtilities.truncate(data[i].zseWarehouseShopId, 32);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 32);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 32);        data[i].zseShopId = FormatUtilities.truncate(data[i].zseShopId, 32);}
    }
    return data;
  }
  ...
  private String getParentID(VariablesSecureApp vars, String strZSE_Warehouse_Shop_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|ZSE_Warehouse_Shop_ID", data[0].getField("zseWarehouseShopId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));
    }
