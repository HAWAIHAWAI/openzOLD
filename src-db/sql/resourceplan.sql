...
for v_cur in (select p_color,p_eventname, p_textcolor, p_correlation from c_getemployeeeventCorrelation(p_user, p_workdate) 
              union all 
...
CREATE OR REPLACE FUNCTION zssi_resourceplanaggregareResourceInTime(firstevent timestamp,lastevent timestamp,pc_bpartner_id varchar,pma_machine_id varchar,pad_org_id varchar)
RETURNS void AS
$_$ 
DECLARE
    datecount timestamp;
...
BEGIN
    ...
    -- Bckwart till we do not find anything..
    ...
    WHILE datecount<= dateendcount LOOP
       ...
       if v_cellcontent!=v_oldplanned or v_indexplanned=10 then    
          if startedp='Y'  and startcontentp!='' then
              ...
              values( get_uuid(),v_cells,v_countbr,datesituationp,'Y',pc_bpartner_id,pma_machine_id,pad_org_id,v_indexplanned);
          end if;
          startedp:='Y';
          v_indexplanned:=1;
        ...
        if v_cellcontent!=v_oldstarted or v_indexstarted=10 then                
           if started='Y' and startcontent!='' then
              ...
              insert into zssi_resourceplan(zssi_resourceplan_id, content, countbr, resourcedate ,
                                            includesplanned,c_bpartner_id, ma_machine_id,ad_org_id,zindex)
              values( get_uuid(),v_cells,v_countbr,datesituation,'N',pc_bpartner_id,pma_machine_id,pad_org_id,v_indexstarted);
            end if;
            started:='Y';
            v_indexstarted:=1;
...
*****************************************************/
    firstevent timestamp:=to_date('01.03.2014','dd.mm.yyyy');
    lastevent  timestamp:=to_date('02.06.2014','dd.mm.yyyy');
    v_cur record;
BEGIN  
truncate table zssi_resourceplan;
...
select greatest((select max(coalesce(dateto,datefrom)) from ma_machineevent),(select max(coalesce(dateto,datefrom)) from C_bpartneremployeeEVENT),(select max(enddate) from c_projecttask)) into lastevent;
  ...
  LOOP
     PERFORM zssi_resourceplanaggregareResourceInTime(firstevent,lastevent,v_cur.c_bpartner_id,v_cur.ma_machine_id,v_cur.ad_org_id);
  END LOOP;
END;
$_$ LANGUAGE plpgsql VOLATILE COST 100;  
...
CREATE OR REPLACE FUNCTION zssi_resourceplanfeedback_trg () RETURNS trigger
LANGUAGE plpgsql
AS $_$ DECLARE 
...
BEGIN
    ...
    if TG_OP = 'DELETE' then
        if old.c_calendarevent_id is not null then
            PERFORM zssi_resourceplanupdate(old.ma_machine_id,old.ad_user_id,old.workdate,null);
        end if;
    end if;
    if TG_OP = 'INSERT' then
        if new.c_calendarevent_id is not null then
            PERFORM zssi_resourceplanupdate(new.ma_machine_id,new.ad_user_id,new.workdate,null);
        end if;
    end if;
    if TG_OP = 'UPDATE' then
        if coalesce(new.c_calendarevent_id,'')!=coalesce(old.c_calendarevent_id,'') then
            PERFORM zssi_resourceplanupdate(new.ma_machine_id,new.ad_user_id,least(old.workdate,new.workdate),greatest(old.workdate,new.workdate));
        end if;
    end if;
-- Updating
...
select zsse_DropTrigger ('zssi_resourceplanfeedback_trg','zspm_ptaskfeedbackline');

CREATE TRIGGER zssi_resourceplanfeedback_trg
  AFTER INSERT OR UPDATE OR DELETE
  ON zspm_ptaskfeedbackline FOR EACH ROW
  EXECUTE PROCEDURE zssi_resourceplanfeedback_trg();
