...
DELETE FROM c_project WHERE
	c_project.c_project_id = old.c_project_id;
...
BEGIN
 ...
 -- RAISE NOTICE '% (BEFORE INSERT): Vorgangsbeziehung von % zu % erstellt', TG_NAME, NEW.dependsontask, NEW.zssm_productionplan_task_id;
   -- nur zur Programmentwicklung - Feld 'zssm_productionplan_taskdep.description' kann später wieder entfernt werden - MH 19.10.12
    ...
    IF (isempty(NEW.description)) THEN
...
SELECT
	...
	case when coalesce(p.projectstatus,'nix') = 'OR' and zspm_projecttaskbom.receiving_locator is not null then
                    ...
                    where m_locator_id=zspm_projecttaskbom.receiving_locator),null)
    ...
    -- c_projecttask_id must exist before inserting into zspm_projecttaskbom
      IF (NEW.isactive = 'Y')  THEN                 -- c_projecttask
        IF (isempty(NEW.c_project_id)) THEN         -- ist Baukasten
          ...
          END IF;
        END IF;
        if (coalesce(NEW.issuing_locator,'')!=coalesce(old.issuing_locator,'')) or (coalesce(NEW.receiving_locator,'')!=coalesce(old.receiving_locator,'')) then 
              update zspm_projecttaskbom set issuing_locator=new.issuing_locator,receiving_locator=new.receiving_locator where c_projecttask_id=NEW.c_projecttask_id ;
        end if;
      END IF;
    END IF; -- TG_OP = 'INSERT'
...
BEGIN
 ...
 -- RAISE NOTICE '% %', ' SQL-PROC : zssm_Copy_ProductionPlan2Project', v_message;
   ...
   -- RAISE NOTICE '%', 'LOOP update each projecttaskbom_id - fire update-trigger zspm_projecttask_trg() to set zspm_projecttaskbom.quantity from qty';
      ...
      LOOP
            if cur_projecttask.dependson is null then
   ...
   -- RAISE NOTICE '%', 'END LOOP';
      ...
      datefinish= (select max(enddate) from c_projecttask where c_project_id=p_ProductionOrder_ID) where c_project_id=p_ProductionOrder_ID;
...
--          v_zspm_projecttaskbom.date_plan := now();
 --       END IF;
    ...
    INSERT INTO zspm_projecttaskdep (
      ...
      dependsontask, c_projecttask_id, stockRotation) -- Vorgaenger, Nachfolger, Lagerbewegung
    SELECT
...
CREATE or replace FUNCTION zssm_getproductionplanofproduct(p_product character varying) returns setof zssm_productionplan_v
AS $_$
DECLARE
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
BEGIN
  ...
  for v_cur in (select * from zssm_productionplan_task_v where m_product_id=p_product and assembly='Y' and isactive='Y' 
  ...
  if v_return= '' then
     ...
     RETURN QUERY
     EXECUTE 'select * from zssm_productionplan_v where zssm_productionplan_v_id in ('||v_return||') order by isdefault desc';
     RETURN;
...
CREATE or replace FUNCTION zssm_getproductionplanIDofproduct(p_product character varying) returns VARCHAR
AS $_$
DECLARE
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
BEGIN
  -- 1. select all tasks with product AS output 
  for v_cur in (select tv.zssm_productionplan_v_id,p.isdefault,tv.zssm_productionplan_task_id from zssm_productionplan_task_v tv,c_project p 
                       where p.c_project_id=tv.zssm_productionplan_v_id
                        and tv.m_product_id=p_product and tv.assembly='Y' and tv.isactive='Y' 
  ...
  LOOP
    ...
    if v_count=0 then -- no other task depends on this one : it is an exit of the plan
       return v_cur.zssm_productionplan_v_id;
    end if;
  END LOOP;
return '';
END;
...
BEGIN
	...
	while exists (select * from c_projecttask where level = i)
	loop
		for v_workstep in (select * from c_projecttask where level = i)
		loop
...
DELETE FROM c_projecttask WHERE
	c_projecttask_id = old.c_projecttask_id;
