...
/*    } else if (vars.commandIn("DELETE_RELATION")) {
      ...
      String strM_Offer_Prod_Cat_ID = vars.getRequiredInStringParameter("inpmOfferProdCatId");
      String message = deleteRelation(vars, strM_Offer_Prod_Cat_ID, strPM_Offer_ID);
      if (!message.equals("")) {
...
/*
  String deleteRelation(VariablesSecureApp vars, String strM_Offer_Prod_Cat_ID, String strPM_Offer_ID) throws IOException, ServletException {
    log4j.debug("Deleting records");
    Connection conn = this.getTransactionConnection();
    try {
      if (strM_Offer_Prod_Cat_ID.startsWith("(")) strM_Offer_Prod_Cat_ID = strM_Offer_Prod_Cat_ID.substring(1, strM_Offer_Prod_Cat_ID.length()-1);
      if (!strM_Offer_Prod_Cat_ID.equals("")) {
        strM_Offer_Prod_Cat_ID = Replace.replace(strM_Offer_Prod_Cat_ID, "'", "");
        StringTokenizer st = new StringTokenizer(strM_Offer_Prod_Cat_ID, ",", false);
        while (st.hasMoreTokens()) {
          String strKey = st.nextToken();
          if (ProductCategoryData.deleteTransactional(conn, this, strKey, strPM_Offer_ID)==0) {
...
*/
  private ProductCategoryData getEditVariables(Connection con, VariablesSecureApp vars, String strPM_Offer_ID) throws IOException,ServletException {
   ...
   private ProductCategoryData[] getRelationData(ProductCategoryData[] data) {
    if (data!=null) {
      for (int i=0;i<data.length;i++) {        data[i].mOfferProdCatId = FormatUtilities.truncate(data[i].mOfferProdCatId, 10);        data[i].adClientId = FormatUtilities.truncate(data[i].adClientId, 44);        data[i].adOrgId = FormatUtilities.truncate(data[i].adOrgId, 44);        data[i].mOfferId = FormatUtilities.truncate(data[i].mOfferId, 44);        data[i].mProductCategoryId = FormatUtilities.truncate(data[i].mProductCategoryId, 44);}
    }
    return data;
  }
...
String strParamM_Product_Category_ID = vars.getSessionValue(tabId + "|paramM_Product_Category_ID");
    ...
    if (!(strParamM_Offer_ID.equals("") && strParamM_Product_Category_ID.equals(""))) hasSearchCondition=true;
    String strOffset = "0";
  ...
  private int saveRecord(VariablesSecureApp vars, OBError myError, char type, String strPM_Offer_ID) throws IOException, ServletException {
    ...
    if (org.openbravo.erpCommon.utility.WindowAccessData.hasReadOnlyAccess(this, vars.getRole(), tabId)) {
        ...
        strSequence = SequenceIdData.getUUID();
          ...
          CrudOperations.UpdateCustomFields(tabId, data.mOfferProdCatId, vars, this);
        } catch(Exception ex) {
