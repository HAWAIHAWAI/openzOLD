...
public class MaterialReceiptPending extends HttpSecureAppServlet {
    ...
    try {
      ...
      if (!strcOrderLineId.equals("")) {
        ...
        while (st.hasMoreTokens()) {
          ...
          if (!strLastBpartnerId.equals(data[0].cBpartnerId)
            ...
            try {
              MaterialReceiptPendingData.insert(conn, this, strmInoutId, vars.getClient(),
                  ...
                  data[0].adUserId, data[0].salesrepId, data[0].adOrgtrxId, data[0].aAssetId, data[0].cProjectId, data[0].cProjecttaskId,
          ...
          BigDecimal qty2ndUOM =null;
          String strQqty2ndUOM="";
          if (! (dataLine[0].mProductUomId.equals(""))) {
            qty2ndUOM=new BigDecimal(dataLine[0].quantityorder).multiply(new BigDecimal(strQtyordered).divide(new BigDecimal(dataLine[0].qtyordered)));
            strQqty2ndUOM=qty2ndUOM.toString();
          }
          ...
          try {
            MaterialReceiptPendingLinesData.insert(conn, this, strSequenceLine, vars.getClient(),
                ...
                dataLine[0].mAttributesetinstanceId, "N", strQqty2ndUOM,
                dataLine[0].mProductUomId, dataLine[0].cProjectId, dataLine[0].cProjecttaskId, dataLine[0].aAssetId);
          } catch (ServletException ex) {
            myMessage = Utility.translateError(this, vars, vars.getLanguage(), ex.getMessage());
            releaseRollbackConnection(conn);
            return myMessage;
  ...
  private OBError mInoutPost(Connection conn, VariablesSecureApp vars, String strmInoutId, String strAdOrgId)
      throws IOException, ServletException, SQLException {
    String pinstance = SequenceIdData.getUUID();

    OBError myMessage = null;
    myMessage = new OBError();
    if (MaterialReceiptPendingData.isAutomaticActivationConfigured(this, strAdOrgId).equals("Y")){
      try {
