...
BEGIN
    ...
    select count(*)+1 into v_count from ad_customcolumn where ad_table_id=v_table;
    select tablename into v_tablename from ad_table where ad_table_id=v_table;
    
    v_name:='customfield'||v_count;
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE') THEN
    ...
    ELSE
      ...
      UPDATED, UPDATEDBY, ACTION, AD_TAB_ID, CLASSNAME, ISDEFAULT,ad_module_id)
      VALUES (v_ID, new.AD_CLIENT_ID, new.AD_ORG_ID, v_IsActive, TO_DATE(NOW()), new.CREATEDBY,
      TO_DATE(NOW()), new.UPDATEDBY, 'W', new.AD_TAB_ID, 'org.openbravo.erpCommon.ad_callouts.ComboReloads' || new.ad_tab_id, 'N',new.ad_module_id);
    ...
    ELSE
      ...
      ISACTIVE, CREATED, CREATEDBY, UPDATED, UPDATEDBY, AD_MODEL_OBJECT_ID, MAPPINGNAME, ISDEFAULT)
      VALUES (v_ID_MAP, new.AD_CLIENT_ID, new.AD_ORG_ID, v_IsActive, TO_DATE(NOW()), new.CREATEDBY,
      TO_DATE(NOW()), new.UPDATEDBY, v_ID, ('/ad_callouts/ComboReloads' || new.ad_tab_id || '.html'), 'N');
...
CREATE OR REPLACE FUNCTION ad_isFieldInForm(p_field_id character varying, p_otherfieldname character varying)
  RETURNS character varying AS
$BODY$ DECLARE 
/***************************************************************************************************************************************************
The contents of this file are subject to the Mozilla Public License Version 1.1 (the "License"); you may not use this file except in
...
v_exists varchar;
BEGIN
  select c.ad_table_id into v_tab from ad_field f,ad_column c where c.ad_column_id=f.ad_column_id and f.ad_field_id=p_field_id;
  if v_tab is not null then
     select c.ad_column_id into v_exists from ad_column c  where ad_table_id=v_tab and upper(columnname)=upper(p_otherfieldname);
...
BEGIN
  select c.columnname into v_retval from ad_field f,ad_column c,ad_fieldinstance fi 
         where f.ad_column_id=c.ad_column_id and f.ad_tab_id=p_tab_id 
         and f.ad_field_id=fi.ad_field_v_id and fi.isfirstfocusedfield='Y';
...
BEGIN
  select c.columnname into v_retval from ad_field f,ad_column c where f.ad_column_id=c.ad_column_id and f.ad_field_id=p_field_id;
  if v_retval is not null then return v_retval; end if;
...
CREATE OR REPLACE FUNCTION ad_tab_copy (
  ...
  BEGIN
    ...
    LOOP
      ...
      IF (cur_parameter.parametername = 'AD_Tab_ID') THEN
        ...
        RAISE NOTICE '%','  AD_Tab_ID = ' || v_old_tab_id;
      ELSE
        RAISE NOTICE '%','*** Unknown Parameter = ' || cur_parameter.parametername;
      END IF;
      END LOOP;
      RAISE NOTICE '%','  Record_ID=' || v_new_tab_id;
   ...
   --****************************************************************************************
      ...
      FOR v_field IN 
        (SELECT * FROM ad_field WHERE ad_tab_id = v_old_tab_id)
      LOOP
        v_nextno := get_uuid();
       ...
       -- get column_id for target-table 
        ...
        LOOP
         -- update target translations inserted by trigger 
          UPDATE ad_field_trl 
          SET 
            name = v_field_trl.name || '`',
            description = v_field_trl.description || '`',
            help = v_field_trl.help  || '`'
...
BEGIN
  ...
  if (v_count=0 and p_isprocessing='N' and  p_message is not null and p_pinstance_id is not null) then
        ...
        values(p_pinstance_id,pid,p_pinstance_id,p_isprocessing,'0',p_Result,substr(p_Message,1,2000),'C726FEC915A54A0995C568555DA5BB3C','0','0','0');
    ...
    UPDATE AD_PINSTANCE
        ...
        ErrorMsg=substr(p_Message,1,2000)
...
*****************************************************/
  ...
  v_TableDir          VARCHAR(50) ; --OBTG:VARCHAR2--
  v_FieldValue        VARCHAR(50) ; --OBTG:VARCHAR2--
  v_FieldDisplay      VARCHAR(50) ; --OBTG:VARCHAR2--
  v_DisplayValue      CHAR(1):='N';
  v_IsTranslated      CHAR(1):='N';
  v_NewTableName      VARCHAR(60) ; --OBTG:VARCHAR2--
  v_NewColumnName     VARCHAR(60) ; --OBTG:VARCHAR2--
  v_NewReference      VARCHAR(32); --OBTG:VARCHAR2--
  v_NewReferenceValue VARCHAR(32); --OBTG:VARCHAR2--
  v_KeyReference      VARCHAR(32); --OBTG:VARCHAR2--
